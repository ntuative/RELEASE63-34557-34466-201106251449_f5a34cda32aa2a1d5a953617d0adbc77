package com.sulake.habbo.ui.widget.effects
{
   ...
   public class EffectsWidget extends RoomWidgetBase
   {
      ...
      private static const const_1452:int = 320;
      
      private static const const_1450:int = 48;
      ...
      {
         return var_936 as EffectsWidgetHandler;
      }
      ...
      public function open() : void
      {
         ...
         if(!this._view)
         {
            ...
            this._view.x = _loc2_.right + const_1451;
      ...
      public function update() : void
      {
         ...
         {
            _loc1_ = this.var_362.getValue(_loc3_.type) as EffectView;
            if(_loc1_)
            {
               _loc1_.update();
            }
            else
            {
               ...
               this.var_362.add(_loc3_.type,_loc1_);
               this.var_62.addListItem(_loc1_.window);
            }
         }
         _loc4_ = this.var_362.length - 1;
         while(_loc4_ >= 0)
         {
            _loc1_ = this.var_362.getWithIndex(_loc4_);
            if(_loc2_.indexOf(_loc1_.effect) == -1)
            {
               this.var_62.removeListItem(_loc1_.window);
               this.var_362.remove(this.var_362.getKey(_loc4_));
               _loc1_.dispose();
            }
            _loc4_--;
         }
         var _loc5_:int = this.var_62.scrollableRegion.height;
         this.var_62.height = Math.max(Math.min(_loc5_,const_1452),const_1450);
      ...
      {
         this.handler.container.catalog.openCatalogPage(CatalogPageName.const_1240,true);
      }
   }
}
