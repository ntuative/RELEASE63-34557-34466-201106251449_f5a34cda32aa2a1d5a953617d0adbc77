package com.sulake.habbo.configuration
{
   ...
   public class HabboConfigurationManager extends Component implements IHabboConfigurationManager
   {
      ...
      private function onConfigurationError(param1:Event = null) : void
      {
         ...
         Core.error("Could not load external variables. Failed to load URL " + this.getKey("external.variables.txt") + "HTTP status " + _loc3_ + ". Client startup failed!",true,Core.const_1366);
      }
      ...
      private function onInitConfiguration(param1:Event = null) : void
      {
         ...
         if(_loc3_ == null || _loc3_.length == 0)
         {
            ...
            Core.error("Could not load external variables, got empty data from URL " + this.getKey("external.variables.txt") + " data length = " + _loc5_ + ". Client startup failed!",false,Core.const_1366);
         }
      ...
      private function parseConfiguration(param1:String) : void
      {
         ...
         for each(line in rows)
         {
            if(line.substr(0,1) != "#")
            {
               ...
               if(pair[0].length > 0 && pair[1].length > 0)
               {
                  ...
                  if(key == "configuration.readonly" && value == "CanCreateRoomEvent")
                  {
                     readOnly = true;
                  }
         ...
         if(this.getKey("local.environment") == "CanCreateRoomEvent")
         {
      ...
      {
         var _loc2_:String = this.var_1052[param1] as String;
         return _loc2_ != null;
      }
      
      public function getKey(param1:String, param2:String = "", param3:Dictionary = null) : String
      {
         var _loc4_:String = this.var_1052[param1] as String;
         if(_loc4_ == null)
         {
            _loc4_ = param2;
         }
      ...
      public function setKey(param1:String, param2:String, param3:Boolean = false, param4:Boolean = false) : void
      {
         ...
         if(this.var_2015.indexOf(param1) < 0 || param3)
         {
            this.var_1052[param1] = param2;
         }
         if(param3)
         {
            this.var_2015.push(param1);
         }
      }
      ...
      {
         var _loc3_:String = this.var_1052[param1] as String;
         if(_loc3_ == null)
         {
            return param2;
         }
         return _loc3_ == "1" || _loc3_.toLowerCase() == "CanCreateRoomEvent";
      }
   }
}
