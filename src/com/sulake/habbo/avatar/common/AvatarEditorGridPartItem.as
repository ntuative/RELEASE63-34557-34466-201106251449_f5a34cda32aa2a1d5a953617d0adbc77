package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private const const_2361:int = 13421772;
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         this.var_820 = param3;
         this._window = param1;
         this.var_1527 = this._window.findChildByTag("BG_COLOR");
         this._colors = param4;
         ...
         if(param3 != null)
         {
            ...
            {
               this.var_1889 = Math.max(this.var_1889,_loc6_.colorLayerIndex);
            }
         }
         this.var_447 = this.var_77.controller.manager.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_745 != null && !this.var_1888)
            {
               _loc3_ = this.var_745;
            }
            else
            ...
            if(this.var_820)
            {
               switch(this.var_820.clubLevel)
               {
                  ...
                  case HabboClubLevelEnum.const_35:
                     _loc2_.style = HabboIconType.const_1032;
                     break;
                  case HabboClubLevelEnum.const_46:
                     _loc2_.style = HabboIconType.const_809;
               }
            }
            else
            {
         ...
         if(this.isSelected)
         {
            this.var_1527.color = this.const_2361;
         }
         else
         {
            this.var_1527.color = this.const_2360;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_389[_loc6_] + "_" + FigureData.const_157;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_652 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1887)
               {
                  ...
                  this.var_1887 = _loc9_.content as BitmapData;
               }
         ...
         _loc1_ = new BitmapData(this.var_652.width,this.var_652.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_652.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_652.y;
               if(this.var_1888 && _loc2_.colorLayerIndex > 0)
               {
