   ...
   public class HabboFriendBarData extends Component implements IHabboFriendBarData
   {
      ...
      override public function dispose() : void
      {
         if(!disposed)
         {
            if(this.var_1058)
            {
               this.var_1058.release(new IIDHabboConfigurationManager());
               this.var_1058 = null;
            }
            if(this.var_93)
            {
            ...
            if(this.var_86)
            {
               if(!this.var_86.disposed)
               {
                  ...
                  this.var_86.events.removeEventListener(FriendRequestEvent.const_212,this.onFriendRequestEvent);
               }
            ...
            if(this.var_220)
            {
               this.var_220.release(new IIDHabboMessenger());
               this.var_220 = null;
            }
            if(this.var_689)
            {
               this.var_689.release(new IIDHabboEventStream());
               this.var_689 = null;
            }
            this.var_111 = null;
      ...
      {
         return this.var_328.getValue(param1);
      }
      ...
      public function followToRoom(param1:int) : void
      {
         if(this.var_93)
         {
            ...
            this.var_93.getHabboMainConnection(null).send(new EventLogMessageComposer(const_701,const_702,const_1596));
         }
      }
      ...
      public function startConversation(param1:int) : void
      {
         if(this.var_220)
         {
            this.var_220.startConversation(param1);
            events.dispatchEvent(new NewMessageEvent(false,param1));
            if(this.var_93)
            {
               this.var_93.getHabboMainConnection(null).send(new EventLogMessageComposer(const_701,const_702,const_1595));
            }
         }
      }
      ...
      public function findNewFriends() : void
      {
         if(this.var_93)
         {
            ...
            this.var_93.getHabboMainConnection(null).send(new EventLogMessageComposer(const_701,const_702,const_1598));
         }
      }
      ...
      public function toggleFriendList() : void
      {
         ...
         if(this.var_86)
         {
            if(!this.var_86.disposed)
            {
               ...
               if(this.var_93)
               {
                  this.var_93.getHabboMainConnection(null).send(new EventLogMessageComposer(const_1102,const_1594,!!this.var_86.isOpen() ? const_1101 : const_1103));
               }
            }
         }
      }
      ...
      public function toggleMessenger() : void
      {
         if(this.var_220)
         {
            ...
            {
               this.var_220.toggleMessenger();
               if(this.var_93)
               {
                  this.var_93.getHabboMainConnection(null).send(new EventLogMessageComposer(const_1102,const_1597,!!this.var_220.isOpen() ? const_1101 : const_1103));
               }
            }
         }
      }
      ...
      public function toggleOfflineStream() : void
      {
         if(this.var_689)
         {
            if(!this.var_689.disposed)
            {
               this.var_689.visible = !this.var_689.visible;
            }
         }
      }
      ...
      {
         this.var_1058 = param2 as IHabboConfigurationManager;
         this.var_2764 = this.var_1058.getBoolean("friendbar.notifications.enabled",false);
         this.var_2023 = this.var_1058.getBoolean("friendbar.requests.enabled",false);
      }
      ...
      {
         this.var_220 = param2 as IHabboMessenger;
      }
      
      private function onOfflineStreamAvailable(param1:IID, param2:IUnknown) : void
      {
         this.var_689 = param2 as IHabboEventStream;
      }
      ...
      private function onFriendListComponentAvailable(param1:IID, param2:IUnknown) : void
      {
         ...
         this.var_86.events.addEventListener(FriendRequestEvent.const_212,this.onFriendRequestEvent);
      }
      ...
      private function onFriendListUpdate(param1:IMessageEvent) : void
      {
         ...
         {
            _loc3_ = this.var_328.getValue(_loc8_);
            if(_loc3_)
            {
               this.var_328.remove(_loc8_);
               this.var_111.splice(this.var_111.indexOf(_loc3_),1);
            }
         }
         for each(_loc4_ in _loc6_)
         {
            _loc3_ = this.var_328.getValue(_loc4_.id);
            if(_loc3_)
            {
            ...
            else if(_loc4_.online || const_320)
            {
               ...
               this.var_328.add(_loc3_.id,_loc3_);
            }
         }
         for each(_loc4_ in _loc7_)
         {
            if(_loc4_.online || const_320)
            {
               if(this.var_328.getValue(_loc4_.id) == null)
               {
                  _loc3_ = new FriendEntity(_loc4_.id,_loc4_.name,_loc4_.realName,_loc4_.motto,_loc4_.gender,_loc4_.online,_loc4_.followingAllowed,_loc4_.figure,_loc4_.categoryId,_loc4_.lastAccess);
         ...
         {
            this.var_111 = !!const_320 ? this.sortByNameAndOnlineStatus(this.var_111) : this.sortByName(this.var_111);
         }
         events.dispatchEvent(new FriendBarUpdateEvent());
      }
      ...
      private function onNewConsoleMessage(param1:NewConsoleMessageEvent) : void
      {
         ...
         this.var_852 = _loc2_.senderId;
         var _loc3_:Boolean = true;
         if(this.var_220)
         {
            if(this.var_220.isOpen())
            {
               _loc4_ = this.var_220.getActiveConversation();
               if(_loc4_)
               {
                  if(_loc4_.id == this.var_852)
                  {
                     _loc3_ = false;
                  }
               }
         ...
         if(_loc3_)
         {
            events.dispatchEvent(new NewMessageEvent(true,this.var_852));
            this.makeNotification(String(this.var_852),FriendNotification.const_401,null,false,false);
         }
      }
      ...
      private function onRoomInvite(param1:RoomInviteEvent) : void
      {
         ...
         this.var_852 = _loc2_.senderId;
         if(this.var_220 && !this.var_220.isOpen())
         {
            events.dispatchEvent(new NewMessageEvent(true,this.var_852));
            this.makeNotification(String(this.var_852),FriendNotification.const_401,null,true,false);
         }
      }
