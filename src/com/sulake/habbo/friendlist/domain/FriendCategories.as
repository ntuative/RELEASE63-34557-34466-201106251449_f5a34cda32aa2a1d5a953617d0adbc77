package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategories
   {
      ...
      private var var_478:IFriendCategoriesDeps;
      ...
      {
         var _loc2_:int = !!param1.online ? int(param1.categoryId) : int(FriendCategory.const_424);
      ...
      public function onFriendListUpdate(param1:IMessageEvent) : void
      {
         ...
         try
         {
            ...
            for each(updatedFriend in p.updatedFriends)
            {
               ...
               this.var_478.messenger.setFollowingAllowed(updatedFriend.id,updatedFriend.followingAllowed && updatedFriend.online);
               currentlyOnline = this.isFriendOnline(updatedFriend.id);
               if(currentlyOnline && !updatedFriend.online)
               {
                  this.var_478.messenger.setOnlineStatus(updatedFriend.id,updatedFriend.online);
               }
               if(!currentlyOnline && updatedFriend.online)
               {
                  this.var_478.messenger.setOnlineStatus(updatedFriend.id,updatedFriend.online);
                  this.var_478.view.setNewMessageArrived();
               }
      ...
      private function updateCategories(param1:Array) : void
      {
         ...
         this.findCategory(FriendCategory.const_424).received = true;
         this.findCategory(FriendCategory.const_1288).received = true;
